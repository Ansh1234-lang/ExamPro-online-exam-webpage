...
const examData: Record<string, any> = {
  '1': {
    ...
    category: 'SSC', difficulty: 'Medium',
    questions: 100, duration: 60, totalMarks: 200,
    passingMarks: 140, negativeMarking: 0.5,
    attempts: 24500, rating: 4.8,
    description: 'A comprehensive mock test for SSC CGL 2024 covering all four sections — General Intelligence, General Awareness, Quantitative Aptitude and English Comprehension.',
    topics: ['General Intelligence & Reasoning', 'General Awareness', 'Quantitative Aptitude', 'English Comprehension'],
    instructions: [
  ...
  '2': {
    ...
    category: 'UPSC', difficulty: 'Hard',
    questions: 100, duration: 120, totalMarks: 200,
    passingMarks: 110, negativeMarking: 0.66,
    attempts: 18200, rating: 4.9,
    description: 'Full-length UPSC Civil Services Preliminary Examination General Studies Paper 1 mock test with questions from History, Geography, Polity, Economy, Environment and Current Affairs.',
    topics: ['History & Culture', 'Geography', 'Indian Polity', 'Economy', 'Environment & Ecology', 'Current Affairs'],
    instructions: [
      'The exam consists of 100 questions for 200 marks.',
      'Each correct answer carries 2 marks.',
    ...
    <main style={{ minHeight: '100vh', background: '#020617', padding: '32px 24px' }}>
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <h1 style={{ fontSize: '26px', fontWeight: 800, color: 'white', marginBottom: '12px' }}>
            {exam.title}
          </h1>
          ...
          <div style={{
            ...
            {[
              ...
              { icon: '➖', label: 'Negative Marking', value: `-${exam.negativeMarking}` },
              { icon: '👥', label: 'Attempts', value: `${(exam.attempts / 1000).toFixed(1)}k` },
            ].map((stat) => (
              <div key={stat.label} style={{
                ...
                <div style={{ color: '#475569', fontSize: '11px', marginTop: '2px' }}>{stat.label}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '28px' }}>
            <div style={{ display: 'flex', gap: '2px' }}>
              {[1, 2, 3, 4, 5].map((star) => (
                <span key={star} style={{
                  fontSize: '18px',
                  color: star <= Math.round(exam.rating) ? '#f59e0b' : '#1e293b'
                }}>★</span>
              ))}
            </div>
            <span style={{ color: '#f59e0b', fontWeight: 700 }}>{exam.rating}</span>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {exam.topics.map((topic: string) => (
                <div key={topic} style={{
                  ...
                  <span style={{ color: '#94a3b8', fontSize: '14px' }}>{topic}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {exam.instructions.map((inst: string, i: number) => (
                <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
                  ...
                  <span style={{ color: '#94a3b8', fontSize: '13px', lineHeight: 1.6 }}>{inst}</span>
                </div>
              ))}
            </div>
          </div>
